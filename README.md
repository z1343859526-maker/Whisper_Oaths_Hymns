...
世界以 tick 推进，左侧是玩家可操作的叙事区，右侧是"上帝视角"调试面板——同 tick 内每个 NPC 在想什么、导演系统如何仲裁冲突，都实时可见。下图为测试世界（一个房间、三个 NPC）中一次多角色同场景的仲裁记录：`test_man` 推门离开、`player` 紧随其后、`test_robot` 打招呼但玩家已跨出门槛，最终只剩它一人。

![导演系统与多角色仲裁](screenshot/golden-testworld-director.jpg)
...
本项目是作者在「用 LLM 做游戏」这条路上的第三个作品。前两个阶段留下了若干可复用的方法论与工具雏形（截图见 `screenshot/journey/`）：

**第一阶段：猜拳肉鸽**——把 LLM 接进 Godot 的对话与对战循环，玩家与 NPC「老张」互相用心理战猜拳。验证了「大模型能在游戏里扮演一个会记仇、会诈唬的对手」，也踩到了提示词不稳定、对话与规则状态脱节的问题。

![猜拳对局](screenshot/journey/02-cuiqian-battle.jpg)

**第二阶段：节点化游戏内容构建工具**——为了让内容生产摆脱硬编码，做了一套可视化编辑器：世界地图、事件节点、效果编辑器、人物/事件/物品知识网络。这段经历确定了「内容与引擎分离」的设计原则，直接沿用到本项目的模组化架构（`worlds/` 配置驱动）。

![世界地图编辑器](screenshot/journey/07-world-map-editor.jpg)
![事件节点编辑器](screenshot/journey/05-node-event-editor.jpg)

**第三阶段：黄金乡谋杀案（本仓库）**——把前两阶段的经验收敛为一套完整的 NPC 世界模拟引擎：BDI 决策、分层记忆、心智引擎、tick 时钟与全知仲裁。目标从「NPC 会说话」升级为「世界会自己运转」。

> 上述编辑器中还内置了 AI 辅助开发能力（AI 生成事件文案、知识图谱补全等），目前未单独开源，有后续公开计划。
