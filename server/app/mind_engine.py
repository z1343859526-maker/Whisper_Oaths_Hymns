...
# 客套敷衍谎只进记忆不注册（09-09 设计口径：避免"这里很安静"刷进秘密区）。
...
def _is_key_lie(npc_id: str, speech: str) -> bool:
    ...
    不进秘密区，仅留在记忆里（09-09 设计口径：秘密只聚焦"男杀女"主剧情）。
    ...
    try:
        ...
        if speech:
            ...
            if lied:
                ...
                # 只有"关键谎话"才注册为可被戳穿的秘密（09-09 设计口径：客套敷衍谎不该
...
def record_blocked_intent(session_id: str, npc_id: str, tick: int,
    ...
    """记录一次「意图受阻/被拒」的自我认知事实(09-09 设计决定)。
    ...
    一次本函数——明确要求"同轮里多次行为改变都记录下来"，因此**绝不覆盖、逐条累加**。
...
#
# 设计（设计裁决）：NPC 开局对世界一无所知——它"知道的房间"只来自【去过】；
